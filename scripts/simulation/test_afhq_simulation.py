...
# SIM_DIR can be overridden via $AFHQ_SIM_DIR. Defaults to the pyramid output;
# falls back to the legacy 3-level dir if pyramid hasn't been generated yet.
def _resolve_sim_dir():
    import os
    env = os.environ.get("AFHQ_SIM_DIR")
    if env:
        return Path(env)
    pyr = Path("/scratch/ondemand30/jw954/afhq_simulation_pyramid")
    if pyr.exists():
        return pyr
    legacy = Path("/scratch/ondemand30/jw954/afhq_simulation_3level")
    if legacy.exists():
        return legacy
    return Path("/scratch/ondemand30/jw954/afhq_simulation")  # original location

SIM_DIR = _resolve_sim_dir()
...
def main():
    ...
    def test_lut_was_used():
        """Confirm LUT noise injection was active during simulation by two
        independent checks:

          (a) CODE-PATH: simulation_summary.csv must contain pyramid_L*
              columns with non-zero per-level usage, OR a `noise_mode` column
              indicating a LUT was used. (Binary, deterministic.)
          (b) STATISTICAL: spatially-flat patches (8×8 windows where the
              local mean is roughly constant) must show MORE per-pixel
              variance than what pure i.i.d. Bernoulli sampling at the local
              mean rate would produce. We test this by selecting only
              "smooth" windows (low local-mean spatial gradient) so scene
              texture is excluded as an alternative explanation.
        """
        # ----- (a) code-path check ---------------------------------------
        with open(csv_path) as f:
            header = f.readline().strip().split(",")
        pyramid_cols = [c for c in header if c.startswith("pyramid_L")]
        if pyramid_cols:
            # Pyramid mode — check at least some pixels resolved at L=1..16
            with open(csv_path) as f:
                csv_rows = list(csv.DictReader(f))
            total_at_inner = 0
            for r in csv_rows:
                for c in pyramid_cols:
                    if c in r and r[c]:
                        total_at_inner += int(r[c])
            runner.log(f"    [code-path] Total pixels resolved via pyramid across scenes: {total_at_inner:,}")
            assert total_at_inner > 0, (
                "pyramid_L* columns exist but all zero — LUT path was not exercised"
            )
        else:
            # Single-level LUT or no-LUT mode — fall back to statistical only
            runner.log("    [code-path] No pyramid_L* columns; falling back to "
                       "statistical check only.")

        # ----- (b) statistical check (smooth-patch only) -----------------
        ...
        # Spatial gradient of local_mean — large gradient = scene edges (texture)
        gy = np.abs(np.gradient(local_mean, axis=0))
        gx = np.abs(np.gradient(local_mean, axis=1))
        smoothness = gx + gy

        # Smooth-patch mask: low spatial gradient AND mid-range mean
        smoothness_thresh = float(np.percentile(smoothness, 25))   # bottom-quartile flatness
        smooth_mask = ((local_mean > 0.1) & (local_mean < 0.9)
                       & (smoothness < smoothness_thresh))
        n_smooth = int(smooth_mask.sum())
        runner.log(f"    [stat] Smooth mid-range patches: {n_smooth:,} pixels "
                   f"(bottom-quartile spatial gradient)")

        # Pure binomial floor at the local mean rate
        p_clip = np.clip(local_mean, 0.01, 0.99)
        binomial_floor = np.sqrt(p_clip * (1 - p_clip) / n_frames_expected)

        if n_smooth < 1000:
            runner.log("    [stat] Too few smooth patches; statistical check skipped.")
            return  # only the code-path check above gates the result

        observed = float(local_std[smooth_mask].mean())
        floor    = float(binomial_floor[smooth_mask].mean())
        ratio    = observed / max(floor, 1e-9)
        runner.log(f"    [stat] Smooth-patch observed std: {observed:.5f}")
        runner.log(f"    [stat] Pure binomial floor:        {floor:.5f}")
        runner.log(f"    [stat] Ratio (observed/floor):     {ratio:.2f}×")

        # On smooth patches scene texture is suppressed, so excess local
        # std comes from per-pixel noise injection (LUT-FPN). Require ≥1.5×.
        # If LUT not used, observed should track ~1.0×.
        assert ratio >= 1.5, (
            f"Smooth-patch local std ratio {ratio:.2f}× < 1.5×. "
            "Either the LUT was not actually used, or LUT injection is "
            "weaker than expected. Binary check (a) above should already "
            "have caught a code-path failure; this likely means the LUT "
            "injection magnitude is small."
        )

    runner.run("S7: LUT injection visible — code-path + smooth-patch statistical",
               test_lut_was_used)

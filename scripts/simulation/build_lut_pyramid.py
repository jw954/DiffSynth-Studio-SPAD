...
def main():
    ...
    # We save the source fields TWICE: once unprefixed (so provenance.load()
    # validates this artifact directly), and once with `source_*` prefix (so
    # downstream code can still distinguish "where the underlying data came
    # from" vs the pyramid's own build metadata).
    INHERIT = ("n_scenes_used", "n_scenes_requested", "n_scenes_skipped",
               "total_samples", "accumulator_module", "k_short",
               "n_gt_frames", "rotate_k", "hotpix_fix_enabled", "seed",
               "mono_bin", "images_dir", "scene_list_hash")
    for k in INHERIT:
        if k in pp.files:
            save_dict[k] = pp[k]               # unprefixed → provenance.load() finds it
            save_dict[f"source_{k}"] = pp[k]   # prefixed → preserves "from per-pixel LUT" semantics

    # Pyramid's OWN identity (overrides the inherited per-pixel LUT version)
    save_dict["artifact_kind"] = "spatial_pyramid_lut"
    save_dict["version"] = VERSION
    import datetime
    save_dict["build_timestamp"] = datetime.datetime.now().isoformat()
    # Keep the source's version + timestamp under explicit names too
    if "version" in pp.files:
        save_dict["source_version"] = pp["version"]
    if "build_timestamp" in pp.files:
        save_dict["source_build_timestamp"] = pp["build_timestamp"]
    if "artifact_kind" in pp.files:
        save_dict["source_artifact_kind"] = pp["artifact_kind"]

...
def main():
    ...
    ratio_obs = float("nan")  # default if data is unavailable for the comparison
    if ("RAW_empty" in spad and "RAW_OD_07" in spad
        and len(spad["RAW_empty"]["lam_det_per_frame"]) > 0
        and len(spad["RAW_OD_07"]["lam_det_per_frame"]) > 0):
        ...
        print("\n[warn] No valid RAW_empty / RAW_OD_07 measurement; "
              "OD-interpretation cross-check skipped (ratio_obs = NaN).")
...
does not support this.** Observed attenuation from RAW_empty to RAW_OD_07:
  ...
  **{("NaN — measurement skipped" if ratio_obs != ratio_obs else f"{ratio_obs:.2f}×")}**
